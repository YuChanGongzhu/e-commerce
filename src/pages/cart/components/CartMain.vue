...
//删除
const onDeleteMemberCart = async (skuId: string) => {
    uni.showModal({
        title: '是否删除',
...
//修改选中状态
const onChangeSelected = (item: CartItem) => {//选择性防抖？？？
    //前端更新
...
const isAllSelected = computed(() => {
    return cartList.value?.length && cartList.value.every(v => v.selected) //every不会检测空数组，检测到会返回true
})
//全选修改
...
<template>
    <scroll-view scroll-y class="scroll-view">
        ...
        <template v-if="memberStore.profile">
                        ...
                        <view class="goods">
                            ...
                            <view class="count">
                                <vk-data-input-number-box v-model="item.count" :min="1" :max="item.stock"
                                    @change="onChangeCount" :index="item.skuId" />
                            </view>>
                        </view>
                        <!-- 右侧删除按钮 -->
                        <template #right>
                            <view class="cart-swipe-right">
                                <button @tap="onDeleteMemberCart(item.skuId)" class="button delete-button">删除</button>
                            </view>
                        </template>
            ...
            <!-- 吸底工具栏 -->
            <view class="toolbar">
                <text @tap="onChangeSelectedAll" class="all" :class="{ checked: isAllSelected }">全选</text>
                <text class="text">合计:</text>
                <text class="amount">{{ selectedCartPrice }}</text>
                <view class="button-grounp">
                    <view class="button payment-button" @tap="gotoPay" :class="{ disabled: selectedCartCount === 0 }">
                        去结算({{
                            selectedCartCount }}
                        )
                    </view>
